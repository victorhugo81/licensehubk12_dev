...
def create_license():
    ...
    if not data.get("name") or not data.get("contract_id"):
        return jsonify(error="'name' and 'contract_id' are required."), 400
    contract = db.session.get(Contract, data["contract_id"])
    if not contract:
        return jsonify(error="Unknown contract_id."), 400

    lic = License(
        name=data["name"], contract=contract, vendor_id=contract.vendor_id,
...
def update_license(id):
    ...
    data = request.get_json(silent=True) or {}
    for field in ("name", "license_count", "status"):
        if field in data:
