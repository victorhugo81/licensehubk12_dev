...
def test_valid_row_passes(db):
    ...
    csv_text = (
        "license,vendor,school,total_licenses,assigned_licenses,expiration_date,annual_cost\n"
...
def test_cross_row_allocation_exceeding_total_is_an_error(db):
    ...
    preview = validate_csv(io.StringIO(csv_text))
    assert preview.errors == 2  # both rows for the over-subscribed license flagged
...
def test_invalid_records_are_never_imported(db):
    ...
    assert License.query.filter_by(name="GoodApp").first() is not None
    assert License.query.filter_by(name="BadApp").first() is None
...
def test_commit_creates_allocation(db):
    ...
    lic = License.query.filter_by(name="GoodApp").first()
    allocation = LicenseAllocation.query.filter_by(license_id=lic.id, school_id=school.id).first()
