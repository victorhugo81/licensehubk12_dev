...
One blueprint per feature area, matching the nav structure: `auth`, `dashboard`, `licenses` (software + license CRUD, allocations, categories), `imports` (CSV upload/preview/commit), `vendors`, `schools`, `contracts`, `reports`, `users`, `settings`, `notifications`, `audit`, `api`.

`licenses.py` covers both "software" and "license" from the spec — in this data model a Software row *is* the license pool for that title (see Models below), so there's one blueprint rather than two nearly-identical ones.
...
The rule "a software's per-school allocations can never exceed its district license count" lives in `allocation.set_allocation()`, not in a form validator. Every write path — the web UI, CSV import (`app/integrations/csv_import.py`), and any future API/integration — must go through this function so the invariant can't be bypassed. See `tests/test_licenses.py::test_allocation_cannot_exceed_district_total`.
...
| Model | Notes |
|---|---|
| `Software` | Also aliased as `License` (`License = Software`) — one row is one software title's license pool. `assigned_licenses`, `available_licenses`, `utilization_pct`, `cost_per_license`, `unused_license_cost` are computed `@property`s from `license_count` and its `allocations`, not stored columns. Don't add columns for these; don't filter SQL on them. |
| `LicenseAllocation` | Per-school breakdown of a `Software`'s `license_count`. One row per (software, school) pair, enforced by a unique constraint. Never insert/update directly — use `app/services/allocation.py`. |
